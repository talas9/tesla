...
# Embedded file name: /firmware/components/odin/src/odin/core/ap/updater.py
...
async def run_command(command, params=None, timeout=10, ap_b=False):
    if ap_b:
        host = settings.APB_IP
    else:
        host = settings.APE_IP
    url = "http://{}:{}/{}".format(host, settings.PORTS["ape_updater"], command)
...
async def read_signature_turbo_b() -> dict:
    results = await run_command("readsig", ap_b=True)
    return dict([line.split(": ") for line in results.splitlines])
